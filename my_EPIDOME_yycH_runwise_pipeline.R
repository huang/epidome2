...
for(sam in sample.names) {
  ...
  merger <- mergePairs(ddF, derepF, ddR, derepR, minOverlap=10, maxMismatch=1)  #1-->0
  mergers[[sam]] <- merger
}
...
saveRDS(seqtab, "yycH_seqtab_from_dada2.rds")
write.csv2(seqtab, "yycH_seqtab_from_dada2.csv")
#14*263-->14*44
seqtab.nochim <- removeBimeraDenovo(seqtab, method="consensus", multithread=TRUE)
saveRDS(seqtab.nochim, "yycH_seqtab_nochim.rds")
write.csv2(seqtab.nochim, "yycH_seqtab_nochim.csv")

#yycH_seqtab_from_dada2.rds
save.image("yycH_seqtab_image.RData")
...
write.csv2(track, "yycH_track.csv")
